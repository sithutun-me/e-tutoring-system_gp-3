            ...
            <div class="dashboard-content px-3 pt-4">
                ...
                <div class="post mt-4">
                    <div class="text-center fit mb-2">
                        <a href="{{ route('tutor.createpost') }}" class="post-btn btn btn-primary shadow-none mb-3"
                            style="background-color: #004AAD;">+ Start a post</a>
                    </div>
                                ...
                                <div class="col-md-3 mb-2 d-flex justify-content-center align-items-center">
                                    <select class="form-select" id="postBy" name="post_by"
                                        ...
                                        <option value="myPosts" {{ request('post_by') == 'myPosts' ? 'selected' : '' }}>My
                                            Posts</option>
                                        <option value="studentPosts"
                                            {{ request('post_by') == 'studentPosts' ? 'selected' : '' }}>Student Posts
                                        </option>
                                    </select>
                                </div>
                                    ...
                                    <select class="form-select form--control" name="student_filter" id="studentFilter"
                                        ...
                                        @foreach ($students as $student)
                                            <option value="{{ $student->id }}"
                                                {{ request('student_filter') == $student->id ? 'selected' : '' }}>
                        ...
                        <div class="post-container">
                            ...
                            @if ($post->is_meeting != 1)
                                @if ($post->creator->id == $user->id)
                                    <div class="edit-btn text-center fit">
                                        <a href="{{ route('tutor.editpost', $post->id) }}"
                                            class="edit-btn btn btn-primary shadow-none"
                                            style=" width: 100px; background-color: #004AAD;">Edit</a>
                                        <a href="#" class="delete-btn btn  shadow-none" data-id="{{ $post->id }}"
                                            style=" width:50px; background-color:#d9534f "><i
                            ...
                            <p>
                                <i class="fa-solid fa-circle-user me-3"
                                    style="font-size: 35px; color:#808080; vertical-align: middle;"></i>
                                <strong class="name me-4"
                                    style="vertical-align: middle; font-size: 1rem; font-family:'Poppins';">{{ $post->creator?->first_name }}
                                    {{ $post->creator?->last_name }}</strong>
                                <span class="date me-1"
                                    style="vertical-align: middle; font-family:'Poppins';">{{ \Carbon\Carbon::parse($post->updated_at)->format('d M Y') }}</span>
                                <span class="time me-4"
                                    style="vertical-align: middle; font-family:'Poppins';">{{ \Carbon\Carbon::parse($post->updated_at)->format('h:i A') }}</span>
                            ...
                            @foreach ($post->documents as $document)
                                <div class="file-attachment d-flex row mx-1" id="docFile">
                                    <div class="d-flex  align-items-center">
                                        ...
                                        <a href="{{ asset($document->doc_file_path) }}"
                            ...
                            <div class="comments pb-0" id="commentsSection">
                                    ...
                                    <div class="comment-item">
                                        ...
                                        <div class="row">
                                            <div class="col-md-5 d-flex align-items-center">
                                                <i class="fa-solid fa-circle-user me-2"
                                                    style="font-size: 20px; color:#808080; vertical-align: middle;"></i>
                                                <strong class="name me-4"
                                                    style="vertical-align: middle; font-size: 1rem">{{ $comment->user->first_name }}
                                            ...
                                            @if ($comment->user_id == $user->id)
                                                <div class="dropdown col-md-1">
                                                    ...
                                                    <ul class="dropdown-menu">
                                                        <li><a class="edit-comment dropdown-item"
                                                                data-id="{{ $comment->id }}"
                                                                data-text="{{ $comment->text }}"
                                                                onclick="commentAction('Edit')">Edit</a></li>
                                                        <li><a class="delete-comment dropdown-item text-danger"
                                                                data-id="{{ $comment->id }}"
                                                                onclick="commentAction('Delete')">Delete</a></li>
                                                    </ul>
                                                </div>
                                            @endif
                                        </div>
                                ...
                                <button class="btn btn-primary show-more-btn mt-2" data-post-id="{{ $post->id }}"
                                    style="display: none;">See More</button>
                                <button class="btn btn-primary show-less-btn mt-2" data-post-id="{{ $post->id }}"
                                    style="display: none;">See Less</button>
                            </div>


                            <form action="{{ route('tutor.postcomment', $post->id) }}" method="POST"
                                enctype="multipart/form-data" id="commentForm_{{ $post->id }}" class="comment-form">
                                @csrf
                                <div class="d-flex align-items-center gap-2 mt-4">
                                    <input type="text" id="replyInput" name="comment" class="form-control"
                                        placeholder="Reply" style="max-width: 1100px;">
                                    <button type="submit" class="btn btn-primary ms-3" style="width: 110px;"
                                        onclick="checkReply('{{ $post->id }}')">Send</button>
                                </div>
                            </form>
                        </div>
                    @empty
            ...
            <div class="modal-content">
                ...
                <form id="deleteForm" method="POST" action="">
                    ...
                    <div class="modal-footer ">
                        <button type="submit" class="btn btn-primary" style="background-color: #004AAD; width: 90px;"
                            id="deleteConfirm">Confirm</button>
                    ...
                    showLessBtn.addEventListener("click", function() {
                        comments.forEach((comment, index) => {
                            if (index >= 3) {
                                ...
                                "none"; // Hide comments beyond the first 3
                            }
                        });
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            document.querySelectorAll(".delete-btn").forEach(button => {
                button.addEventListener("click", function(event) {
                    ...
                    let deleteUrl = `{{ route('tutor.deletepost', ':id') }}`.replace(':id',
                    postId);
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            document.querySelectorAll('.delete-comment').forEach(button => {
                button.addEventListener('click', function(event) {
                    ...
                    let commentId = this.dataset.id;
                    let deleteUrl = `{{ route('tutor.deletecomment', ':id') }}`.replace(':id',
                        commentId);
